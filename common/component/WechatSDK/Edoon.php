...
class Edoon
{
	...
	const OAUTH_TOKEN_PREFIX = 'http://edooon.com';
	const OAUTH_TOKEN_URL = '/openapi/oauth2/token';
	const OAUTH_REFRESH_URL = '/oauth2/refresh_token?';
	const OAUTH_USERINFO_URL = 'http://edooon.com/openapi/oauth2/operate/get_user_info';
	...
	{
		$options = \Yii::$app->params['edoon_config'];
        return new Edoon($options);
	}
	/**
...
// 	        'response_type' => 'code'
// 	    );
	    \Yii::$app->response->redirect('http://edooon.com/openapi/oauth2/authorize?client_id='.$this->appid.'&redirect_uri='.rawurlencode($callback).'&state=auth&response_type=code');
// 		$this->redirect_post(self::OAUTH_PREFIX.self::OAUTH_AUTHORIZE_URL, $data);
	}
	...
	public function getOauthAccessToken($code=null){
		...
		$param = array(
		    'client_id' => $this->appid,
		    'redirect_uri' => 'http://wechat.paobuqu.com/edoon/user',
		    'grant_type' => 'authorization_code',
		    'client_secret' => $this->appsecret,
		    'code' => $code
		);
		$result = $this->http_post(self::OAUTH_TOKEN_PREFIX.self::OAUTH_TOKEN_URL,$param);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json) {
	...
	public function getOauthUserinfo($access_token){
		...
		$result = $this->http_post(self::OAUTH_USERINFO_URL, $param);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json) {
