<?php
namespace common\component\WechatSDK;
...
class Xiaomi
{
	...
	const OAUTH_PREFIX = 'http://edooon.com';
	const OAUTH_AUTHORIZE_URL = '/openapi/oauth2/authorize';
	const OAUTH_TOKEN_PREFIX = 'https://account.xiaomi.com';
	const OAUTH_TOKEN_URL = '/oauth2/token';
	const OAUTH_REFRESH_URL = '/oauth2/refresh_token?';
	const OAUTH_USERINFO_URL = 'https://open.account.xiaomi.com/user/profile';
	...
	const HUAMI_DATA = 'https://hmservice.mi-ae.com.cn/user/summary/getData?';
	...
	{
		$options = \Yii::$app->params['xiaomi_config'];
        return new Xiaomi($options);
	}
	/**
	...
	*/
	...
	public function getOauthRedirect($callback){
	    \Yii::$app->response->redirect('https://account.xiaomi.com/oauth2/authorize?client_id='.$this->appid.'&redirect_uri='.rawurlencode($callback).'&response_type=code');
	}
	 ...
	 * @return array {access_token,token_type,expires_in,refresh_token,scope}
	 */
	public function getOauthAccessToken($code=null){
		if( is_null($code) ){
			$code = isset($_GET['code'])?$_GET['code']:'';
		}
		if (!$code) return false;
		$result = $this->http_get(self::OAUTH_TOKEN_PREFIX.self::OAUTH_TOKEN_URL."?client_id=".$this->appid."&redirect_uri=".rawurlencode('http://wechat.paobuqu.com/xiaomi/user')."&client_secret=".$this->appsecret."&grant_type=authorization_code&code=".$code."&token_type=mac");
		if ($result)
		{
	...
	public function getOauthRefreshToken($refresh_token){
		$result = $this->http_post(self::OAUTH_TOKEN_PREFIX.self::OAUTH_REFRESH_URL.'appid='.$this->appid.'&grant_type='.$refresh_token.'&refresh_token=REFRESH_TOKEN');
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 */
	public function getOauthUserinfo($access_token,$macKey){
	    $nonce = XMUtil::getNonce();
	    $params = array();
	    if(!array_search('clientId', $params)) {
	        $params['clientId'] = $this->appid;
	    }
	    if( !array_search('token', $params)) {
	        $params['token'] = $access_token;
	    }
	    $sign = XMUtil::buildSignature($nonce, 'GET', 'open.account.xiaomi.com', '/user/profile', $params, $macKey);
	    $header =XMUtil::buildMacRequestHead($access_token, $nonce, $sign);
		$result = $this->http_get(self::OAUTH_USERINFO_URL."?clientId=".$this->appid."&token=".$access_token,$header);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json) {
...
// 				$this->errMsg = $json['errmsg'];
	...
	//获取小米运动数据
	public function getXiaomiData($param)
	{
	    $third_appid = \Yii::$app->params['huami_config']['third_appid'];
	    $third_appsecret = \Yii::$app->params['huami_config']['third_appsecret'];
	    $result = $this->http_get(self::HUAMI_DATA.'appid='.$this->appid.'&third_appid='.$third_appid.'&third_appsecret='.$third_appsecret.'&mac_key='.$param['mac_key'].'&call_id='.time().'&access_token='.$param['access_token'].'&fromdate='.$param['fromdate'].'&todate='.$param['todate'].'&v=1.0&l=english');
	    if ($result)
	    {
	        $json = json_decode($result,true);
	        if (!$json || $json['code']!=1) {
