...
class Codoon
{
    ...
    const OAUTH_PREFIX = ' https://openapi.codoon.com';

    const OAUTH_AUTHORIZE_URL = '/authorize?';

    const OAUTH_TOKEN_PREFIX = 'http://api.codoon.com';

    const OAUTH_TOKEN_URL = '/token';

    const OAUTH_REFRESH_URL = '/token';

    const OAUTH_USERINFO_URL = 'http://api.codoon.com/api/verify_credentials';

    const OAUTH_MILEAGE_URL = 'http://api.codoon.com/api/get_route_by_id';
    ...
    {
        $options = \Yii::$app->params['codoon_config'];
        return new Codoon($options);
    }
     ...
     * @return string
     */
    public function getOauthRedirect($callback, $scope = 'user,sports')
    {
        return self::OAUTH_PREFIX . self::OAUTH_AUTHORIZE_URL . 'client_id=' . $this->appid . '&redirect_uri=' . rawurlencode($callback) . '&response_type=code&scope=' . $scope;
    }
    ...
    public function getOauthAccessToken($code = null)
    {
        ...
        $param = array(
            ...
            'redirect_uri' => 'http://wechat.paobuqu.com/codoon/user',
            'code' => $code,
            'scope' => 'user,sports'
        );
        $result = $this->http_post(self::OAUTH_TOKEN_PREFIX . self::OAUTH_TOKEN_URL, $param, $header);
        if ($result) {
            $json = json_decode($result, true);
            if (! $json || ! empty($json['error_code'])) {
                $this->errCode = $json['error_code'];
    ...
    public function getOauthRefreshToken($refresh_token)
    {
        ...
        $param['scope'] = 'user,sports';
        $result = $this->http_post(self::OAUTH_TOKEN_PREFIX . self::OAUTH_REFRESH_URL, $param);
        if ($result) {
            $json = json_decode($result, true);
            if (! $json || ! empty($json['error_code'])) {
                $this->errCode = $json['error_code'];
     ...
     * @return array {nickname,sex,province,city,country,headimgurl,privilege}
     */
    public function getOauthUserinfo($access_token)
    {
        $header[] = "Authorization:Bearer " . $access_token;
        $result = $this->http_get(self::OAUTH_USERINFO_URL, $header);
        if ($result) {
            $json = json_decode($result, true);
            if (! $json || ! empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array {total_time,upload_time,start_time,route_image,end_time,total_length}
     */
    public function getOauthMileage($access_token, $route_id)
    {
        $param = array(
            'route_id' => $route_id
        );
        $header[] = "Authorization:Bearer " . $access_token;
        $result = $this->http_post(self::OAUTH_MILEAGE_URL, json_encode($param), $header);
        if ($result) {
            $json = json_decode($result, true);
            if (! $json || ! empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
