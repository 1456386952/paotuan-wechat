...
final class MessageApi extends Object
{
	static private $username = "runningtogether@shnfxx";
	static private $password = "Running2014";
	...
	static private $messages = array(
        ...
        /*发送短信*/
	static public function send($to, $text)
	{
            $text = mb_convert_encoding($text, 'GB2312', 'UTF-8');
            $code = Msg_PostSingle(self::$username, self::$password, $to, $text, "");
            if( $code !== 0 ){
	...
	const CODE_LIFETIME = 1800;
	const CODE_SESSION_KEY = 'message.code';

	static public function createCode($lifetime=self::CODE_LIFETIME)
	{
		if( isset(yii::app()->session[self::CODE_SESSION_KEY]) ){
			if( yii::app()->session[self::CODE_SESSION_KEY]['expiretime'] < time() ){
				$generate = yii::app()->session[self::CODE_SESSION_KEY]['value'];
			}
		}
		if( empty($generate) ){
			$generate = "";
		...
		yii::app()->session[self::CODE_SESSION_KEY] = array(
	...
	static public function checkCode($code)
	{
		if( isset(yii::app()->session[self::CODE_SESSION_KEY]) ){
			if( yii::app()->session[self::CODE_SESSION_KEY]['expiretime'] > time() ){
				return yii::app()->session[self::CODE_SESSION_KEY]['value'] == $code;
			}
		}
	}
}
...
$Msg_WebServerIP = "211.147.224.154";  	//接收信息的服务器IP
...
function _Msg_FinalPostMessage($userName, $password, $to, $text, $subId)
{
	...
	$Msg_GatewayServer = "211.147.224.154:13013";		//发送信息的服务器，网关服务器	
	$httpRequest = sprintf(POST_STRING, $Msg_GatewayServer, $userName, $password,
		$to, $textUrl, $subId);

	//print($httpRequest);
	$result = @file_get_contents($httpRequest);
	if ( $result !== "0"){ //发送信息
		return $result === false ? -101 : -99;
	}else{
		return 0;	//发送成功
	}
}
